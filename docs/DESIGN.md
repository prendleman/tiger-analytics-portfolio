...
- **Scripts + notebooks**: Every notebook has a corresponding script (e.g. `readmission_model.py`, `time_series_forecast.py`) so the pipeline and Airflow DAG can run without Jupyter. **Readmission** feature/label logic lives in `readmission_features.py` and is shared by the script and `readmission_prediction.ipynb`. Ranking metrics and temporal splitting are in `readmission_evaluation.py`.
